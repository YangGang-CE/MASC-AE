...
def train_model_single_stage(model, train_loader, val_loader, optimizer, device, 
                             num_epochs=100, save_dir='checkpoints', log_interval=10, 
                             patience=10, delta=1e-4, stress_weight=1.0, mask_weight=0.1, 
                             mask_threshold=0.3776, lr_scheduler=None, mse_weight=1.0, 
                             ssim_weight=1.0, gradient_weight=1.0, focal_weight=0.5, 
                             dice_weight=0.5):
    """
    单阶段训练多任务AutoEncoder模型
    
    从训练开始到结束的每一个轮次（Epoch），模型的所有部分（编码器、stress解码器、mask解码器）
    都同时参与训练和参数更新。
    """
    ...
    print(f"\n开始单阶段训练 (Epochs 1-{num_epochs}): 同时训练编码器、应力解码器和mask解码器")
    print("=" * 80)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_pbar):
            ...
            # 生成failure mask
            mask_gt = generate_failure_mask(target, threshold=mask_threshold).to(device)
            ...
            # 使用多任务损失函数
            total_loss, stress_loss, mask_loss, stress_mse_loss, stress_ssim_loss = multi_task_loss_function(
                outputs, target, mask_gt,
            ...
            train_stress_loss_sum += stress_loss.item()
            train_mask_loss_sum += mask_loss.item()
            train_ssim_loss_sum += stress_ssim_loss.item()
            
            train_pbar.set_postfix({
                'total_loss': total_loss.item() / len(data),
                'stress_loss': stress_loss.item() / len(data),
                'mask_loss': mask_loss.item() / len(data),
                'ssim_loss': stress_ssim_loss.item() / len(data)
            })
        ...
        with torch.no_grad():
            ...
            for data, target in val_pbar:
                ...
                # 使用多任务损失函数
                total_loss, stress_loss, mask_loss, stress_mse, stress_ssim_loss = multi_task_loss_function(
                    outputs, target, mask_gt,
                    stress_weight=stress_weight,
                    mask_weight=mask_weight,
                    mse_weight=mse_weight,
                ...
                val_stress_loss_sum += stress_loss.item()
                val_mask_loss_sum += mask_loss.item()
                val_ssim_loss_sum += stress_ssim_loss.item()
        ...
        avg_train_stress_loss = train_stress_loss_sum / len(train_loader.dataset)
        avg_train_mask_loss = train_mask_loss_sum / len(train_loader.dataset)
        avg_train_ssim_loss = train_ssim_loss_sum / len(train_loader.dataset)
        
        avg_val_loss = val_total_loss / len(val_loader.dataset)
        avg_val_stress_loss = val_stress_loss_sum / len(val_loader.dataset)
        avg_val_mask_loss = val_mask_loss_sum / len(val_loader.dataset)
        avg_val_ssim_loss = val_ssim_loss_sum / len(val_loader.dataset)
        
        # 记录历史
        history['train_loss'].append(avg_train_loss)
        history['train_stress_loss'].append(avg_train_stress_loss)
        history['train_mask_loss'].append(avg_train_mask_loss)
        history['train_ssim_loss'].append(avg_train_ssim_loss)
        history['val_loss'].append(avg_val_loss)
        history['val_stress_loss'].append(avg_val_stress_loss)
        history['val_mask_loss'].append(avg_val_mask_loss)
        history['val_ssim_loss'].append(avg_val_ssim_loss)
        
        # TensorBoard记录
        writer.add_scalar('Loss/Train_Total', avg_train_loss, epoch)
        writer.add_scalar('Loss/Train_Stress', avg_train_stress_loss, epoch)
        writer.add_scalar('Loss/Train_Mask', avg_train_mask_loss, epoch)
        writer.add_scalar('Loss/Train_SSIM', avg_train_ssim_loss, epoch)
        writer.add_scalar('Loss/Val_Total', avg_val_loss, epoch)
        writer.add_scalar('Loss/Val_Stress', avg_val_stress_loss, epoch)
        writer.add_scalar('Loss/Val_Mask', avg_val_mask_loss, epoch)
        writer.add_scalar('Loss/Val_SSIM', avg_val_ssim_loss, epoch)
        ...
        if avg_val_loss < best_val_loss - delta:
            ...
            # 保存最佳模型
            best_model_path = os.path.join(save_dir, 'best_model_single_stage.pth')
            checkpoint = {
                'epoch': epoch,
            ...
            print(f'最佳模型保存于 epoch {epoch+1}，val_loss: {best_val_loss:.6f}')
        else:
            patience_counter += 1
        ...
        if (epoch + 1) % log_interval == 0:
            ...
            print(f'Train Loss: {avg_train_loss:.6f} (Stress: {avg_train_stress_loss:.6f}, Mask: {avg_train_mask_loss:.6f}, SSIM: {avg_train_ssim_loss:.6f})')
            print(f'Val Loss: {avg_val_loss:.6f} (Stress: {avg_val_stress_loss:.6f}, Mask: {avg_val_mask_loss:.6f}, SSIM: {avg_val_ssim_loss:.6f})')
            ...
            # 可视化结果
            visualize_results(model, val_loader, device, save_dir, epoch, mask_threshold)
        
        # 早停
        if patience_counter >= patience:
            print(f'早停触发于 {epoch+1} epochs')
            break
    ...
    # 保存最终模型
    final_model_path = os.path.join(save_dir, 'final_model_single_stage.pth')
    checkpoint = {
        'epoch': epoch,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
# def visualize_results(model, data_loader, device, save_dir, epoch, mask_threshold=1e-3):
#     """可视化结果，包括应力预测和mask预测"""
...
#     # 清理GPU内存
#     del outputs, stress_pred, mask_pred, mask_gt
#     torch.cuda.empty_cache()
    
#     n_samples = min(3, data_np.shape[0])
#     fig, axes = plt.subplots(5, n_samples, figsize=(4*n_samples, 20))
    
#     for i in range(n_samples):
#         # 输入裂纹图
#         axes[0, i].imshow(data_np[i, 0], cmap='viridis')
#         axes[0, i].set_title('Input Crack')
#         axes[0, i].axis('off')
        
#         # 真值应力图
#         axes[1, i].imshow(targets_np[i, 0], cmap='viridis')
#         axes[1, i].set_title('GT Stress')
#         axes[1, i].axis('off')
        
#         # 预测应力图
#         axes[2, i].imshow(stress_pred_np[i, 0], cmap='viridis')
#         axes[2, i].set_title('Pred Stress')
#         axes[2, i].axis('off')
    
#     plt.tight_layout()
#     plt.savefig(os.path.join(img_dir, f'comparison_epoch_{epoch}.png'), dpi=150, bbox_inches='tight')
#     plt.close()
    
#     # 清理内存
#     del data_np, targets_np, stress_pred_np
#     gc.collect()
...
def plot_history(history, save_dir):
    ...
    plt.plot(history['train_loss'], label='Training Total Loss')
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'total_loss_history.png'))
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.plot(history['train_stress_loss'], label='Training Stress Loss')
    plt.plot(history['val_stress_loss'], label='Validation Stress Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Stress Loss')
    plt.title('Stress Reconstruction Loss')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, 'stress_loss_history.png'))
    plt.close()
...
def main():
    parser = argparse.ArgumentParser(description='Multi-task AutoEncoder for Von Mises Stress Prediction and Failure Mask Segmentation')
